...
export default function ConnexionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const adminCheck = await fetch(`/api/admin/check?email=${encodeURIComponent(email)}`, {
        cache: 'no-store',
      })
      const payload = (await adminCheck.json()) as { authorized?: boolean }

      setLoading(false)
      router.push(payload.authorized ? '/admin' : '/profil')
    } catch {
      setLoading(false)
      router.push('/profil')
    }
    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-gray-50 dark:bg-dark-bg px-4 py-16">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-dark-card border border-gray-100 dark:border-dark-border rounded-2xl p-8 shadow-sm">
          ...
          {/* OAuth temporairement desactive (Google / Apple).
              Rebrancher ici le divider + boutons quand les providers Supabase seront configures. */}
        </div>
