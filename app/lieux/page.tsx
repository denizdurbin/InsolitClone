...
// Import dynamique (Leaflet ne fonctionne pas côté serveur)
...
export default function LieuxPage() {
  const offersWithCoords = useMemo(() => offers.filter(o => o.coords), [])

  const [selected,     setSelected]     = useState(offersWithCoords[0])
  const [search,       setSearch]       = useState('')
  const [userPosition, setUserPosition] = useState<[number, number] | null>(null)
  const [geoStatus,    setGeoStatus]    = useState<GeoStatus>('idle')
  const [distances,    setDistances]    = useState<Record<string, string>>({})

  // Calculer distance à vol d'oiseau entre 2 coords (Haversine)
  ...
  function requestGeolocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        offersWithCoords.forEach(o => {
          if (!o.coords) return
          const d = haversine(coords[0], coords[1], o.coords[0], o.coords[1])
          dist[o.id] = d < 1 ? `${Math.round(d * 1000)} m` : `${d.toFixed(1)} km`
        })
        setDistances(dist)
      },
      (err) => {
  ...
  const filtered = useMemo(() => {
    ...
    return offersWithCoords.filter(
      o => !q || o.title.toLowerCase().includes(q) || o.categoryLabel.toLowerCase().includes(q)
    )
  }, [offersWithCoords, search])

  // Trier par distance si position connue
  const sorted = useMemo(() => {
    if (!userPosition || Object.keys(distances).length === 0) return filtered
    return [...filtered].sort((a, b) => {
      const da = a.coords ? haversine(userPosition[0], userPosition[1], a.coords[0], a.coords[1]) : 999
      const db = b.coords ? haversine(userPosition[0], userPosition[1], b.coords[0], b.coords[1]) : 999
      return da - db
    })
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-gray-50 dark:bg-dark-bg">
        ...
        <div className="max-w-6xl mx-auto flex flex-wrap items-center gap-3">
          ...
          >
            ...
            {geoStatus === 'loading' ? 'Localisation…'
              ...
              : geoStatus === 'denied' ? 'Accès refusé'
              : geoStatus === 'error'  ? 'Erreur GPS'
              : 'Me localiser'}
          </button>
          ...
          {geoStatus === 'success' && (
            <p className="text-xs text-green w-full">
              ✓ Offres triées par distance depuis ta position
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden max-w-6xl mx-auto w-full px-6 py-4 gap-4">
        ...
        <div className="w-full md:w-72 flex-shrink-0 overflow-y-auto space-y-1.5 pb-2">
          {sorted.map((offer) => {
            const dist = distances[offer.id] || offer.distance
            return (
              <button
                key={offer.id}
                ...
                `}
              >
                <div className={`w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-sm bg-gradient-to-br ${offer.gradient} text-white font-bold`}>
                  {offer.emoji.slice(0, 2)}
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`text-xs font-bold truncate ${selected?.id === offer.id ? 'text-pink' : 'text-gray-800 dark:text-gray-100'}`}>
                    {offer.title}
                  </p>
                ...
                {dist && dist !== '—' && (
                  <div className="flex flex-col items-end flex-shrink-0">
                    <span className="text-[10px] text-gray-400 font-medium">{dist}</span>
        ...
        <div className="flex-1 min-h-[400px] md:min-h-0 relative">
            ...
            <div className="flex flex-col gap-1">
              {Object.entries(categoryColor).map(([cat, color]) => (
                <div key={cat} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-gray-600 dark:text-gray-400 capitalize">{cat === 'activite' ? 'Activité' : cat.charAt(0).toUpperCase() + cat.slice(1)}</span>
                </div>
              ))}
              {userPosition && (
          ...
          {selected && (
            <div className="absolute top-3 right-3 w-64 bg-white dark:bg-dark-card border border-gray-100 dark:border-dark-border rounded-2xl shadow-xl overflow-hidden z-[400]">
              <div className={`h-16 bg-gradient-to-br ${selected.gradient} flex items-center justify-center text-white text-2xl font-black`}>
                {selected.emoji}
              </div>
              <div className="p-4">
                ...
                {distances[selected.id] && (
                  <p className="text-xs text-pink font-semibold mb-2">
                    📍 {distances[selected.id]} de toi
                  </p>
                )}
                <Link
