...
export function OfferCard({ offer, size = 'md' }: OfferCardProps) {
    return (
        ...
        flex flex-col
        `,
                size === 'sm' ? 'h-[260px]' : 'h-[320px]'
            )}
        >
            {/* Image */}
            <div
                className={clsx(
                    'h-1/2 w-full flex items-center justify-center font-black text-white',
            ...
            >
                {offer.emoji?.startsWith('http') ? (
                    ...
                    <span className={clsx('drop-shadow-md', size === 'sm' ? 'text-4xl' : 'text-5xl')}>
            {offer.emoji}
          </span>
                )}
            </div>
            ...
            {/* Body */}
            <div className="h-1/2 p-3.5 flex flex-col justify-between">
                <div>
